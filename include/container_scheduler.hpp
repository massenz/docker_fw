...
class ContainerScheduler: public Scheduler {
 public:
  ...
  ContainerScheduler(
      ...
      : implicitAcknowledgements_(implicitAcknowledgements),
        ...
        config_(config),
        launched_(false),
        pidFilename_("/tmp/scheduler.pid") { }


  virtual ~ContainerScheduler() { }
  ...
  virtual void slaveLost(SchedulerDriver *driver, const mesos::SlaveID &sid) override { }
 ...
 private:
  ...
  void setCommand(mesos::CommandInfo *pCmd);

  // The resources necessary to run an Nginx server on a Mesos Agent node.
