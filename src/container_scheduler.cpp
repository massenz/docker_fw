...
const Resources ContainerScheduler::TASK_RESOURCES =
    Resources::parse("cpus:" + stringify(CPUS_PER_TASK) +
        ";mem:" + stringify(MEM_PER_TASK)).get();

const std::string ContainerScheduler::REV{
    std::to_string(VERSION_MAJOR) + "." +
        std::to_string(VERSION_MINOR)
};


bool ContainerScheduler::validateConfig() {
...
long ContainerScheduler::getPid() {
  long pid;
...
void ContainerScheduler::registered(SchedulerDriver *driver,
                                const FrameworkID &frameworkId,
                                const MasterInfo &masterInfo) {
  cout << "Registered on Master node:  " << masterInfo.hostname()
      << " (" << masterInfo.address().ip() << ':' << masterInfo.port() << ")\n";
}


void ContainerScheduler::reregistered(SchedulerDriver *driver,
                                  const MasterInfo &masterInfo) {
...
void ContainerScheduler::setCommand(mesos::CommandInfo *pCmd) {
  pCmd->set_shell(false);
  pCmd->set_value("docker");
  pCmd->add_arguments("docker");
  pCmd->add_arguments("images");
  pCmd->add_arguments("-a");
  //pCmd->add_arguments(config_);
}


void ContainerScheduler::resourceOffers(SchedulerDriver *driver,
  ...
  vector<TaskInfo> tasks;
      foreach (const Offer &offer, offers) {
          ...
          if (remaining.flatten().contains(TASK_RESOURCES)) {
            LOG(INFO) << "Running Docker container, using offer [" << offer.id()
                      << "] with resources: " << TASK_RESOURCES << endl;

            TaskInfo task;
            task.set_name("NginxContainerTask");
            task.mutable_task_id()->set_value("docker_task-001");
          ...
          if (launched_) {
            ...
            LOG(INFO) << "Containerized Nginx now running on " << offer.hostname() << " ["
                      << offer.slave_id() << "]\n"
...
void ContainerScheduler::statusUpdate(SchedulerDriver *driver,
...
void ContainerScheduler::error(SchedulerDriver *driver, const string &message) {
  cout << message << endl;
}
...
int run_scheduler(const std::string &masterIp,
  ...
  name << "Nginx Framework (rev. " << ContainerScheduler::REV << ")";
  ...
  ContainerScheduler scheduler(implicitAcknowledgements, role, config);
